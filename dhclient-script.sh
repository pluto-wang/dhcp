...
add_default_gateway() {
    ...
    if is_router_reachable ${router} ; then
        ...
        if [ ${interface:0:7} == "riftnic" ]; then
            if [[ ${interface: -1} =~ ^-?[0-9]+$ ]]; then
                tableID="20"${interface: -1}
            elif [[ ${interface: -2} =~ ^-?[0-9]+$ ]]; then
                tableID="2"${interface: -2}
            fi
        elif [ ${interface:0:6} == "iftnic" ]; then
            if [[ ${interface: -1} =~ ^-?[0-9]+$ ]]; then
                tableID="10"${interface: -1}
            elif [[ ${interface: -2} =~ ^-?[0-9]+$ ]]; then
                tableID="1"${interface: -2}
            fi
        else
            rtTable="/etc/iproute2/rt_tables"
            sed -i "/${interface}/d" ${rtTable}
            for((i=10; i< 50; i++)); do
                if ! grep -q "${i}" ${rtTable}; then
                    echo "${i} ${interface}" >> ${rtTable}
                    tableID=${i}
                    break
                fi
            done
    ...
    ip rule delete table ${tableID}
    ip -4 route replace ${mysubnet}/${new_prefix} dev ${interface} table ${tableID}
    ip -4 route replace default via ${router} dev ${interface} table ${tableID}
    ip rule add from ${new_ip_address} table ${tableID}
    if [ ${interface} == "mgmt0" ]; then
